...
class OrderController extends Controller
{
    public function addOrder()
    {
        ...
        return view('orders.add', compact('barang'));
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        if ($getCart) {
            if (array_key_exists($request->kode_barang, $getCart)) {
                $getCart[$request->kode_barang]['qty'] += $request->qty;
    ...
    {
        $customers = Customer::orderBy('nama_barang', 'ASC')->get();
    ...
    public function invoicePdf($invoice)
    {
        $order = Order::where('invoice', $invoice)
            ->with('customer', 'order_detail', 'order_detail.barang')->first();
